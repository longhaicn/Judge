...
public interface OrgnizationDao {
    ...
    @Select("select u.u_id,u.oa_id,u.ding_id,u.u_nickname,u.u_short_name,u.u_username,u.u_email,u.u_status,o.o_role_id as u_role,u.u_department,u.token,u.datetime from t_user u join t_orgnization o on u.u_id = o.o_user_id where o_project_id =#{0} and o_role_id = #{1}")
    List<User> searchUsersByproidAndroleid(int project_id,int roleid);

    @Select("select u.u_id,u.oa_id,u.ding_id,u.u_nickname,u.u_short_name,u.u_username,u.u_email,u.u_status,o.o_role_id as u_role,u.u_department,u.token,u.datetime from t_user u join t_orgnization o on u.u_id = o.o_user_id join t_affair a on o.o_project_id = a.a_project_id where a_id =#{0} and o_user_id = #{1}")
    ...
    @Select("  select * from t_orgnization o where o.o_project_id= #{0} and o.o_user_id in  (select j.j_evaluated_id from t_judge j where j.j_project_id= #{0} and datetime  BETWEEN CONVERT(datetime,#{1} ,120) AND CONVERT(datetime,#{2} ,120) group by j.j_evaluated_id having count(j.j_evaluated_id) >0 )")
    List<Orgnization> selectScoreOrgnization(int project_id, String start, String end);

    @Select("select * from t_orgnization o where o.o_user_id in (select j.j_evaluated_id from t_judge j where datetime  BETWEEN CONVERT(datetime,#{0} ,120) AND CONVERT(datetime,#{1} ,120) group by j.j_evaluated_id having count(j.j_evaluated_id) >0 )")
